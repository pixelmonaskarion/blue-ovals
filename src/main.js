	...
	session.defaultSession.webRequest.onHeadersReceived((details, callback) => {
		callback({
		  responseHeaders: {
			...
			'Content-Security-Policy': [ "default-src 'self'; connect-src 'self' ws://localhost:3000 https://chrissytopher.com:40441; script-src 'self' http://localhost:3000; style-src 'self' 'unsafe-inline';" ]
		  }
		});
	  });
